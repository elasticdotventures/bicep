    ...
    public sealed class TypeAssignmentVisitor : SyntaxVisitor
    {
        ...
        public override void VisitObjectSyntax(ObjectSyntax syntax)
            => AssignType(syntax, () => 
            {
                ...
                var parentObjectType = typeManager.GetDeclaredType(syntax) as ObjectType;
                ...
                var namedProperties = syntax.Properties
                    ...
                    .Select(group => 
                    {
                        if (parentObjectType is not null && 
                            parentObjectType.Properties.TryGetValue(group.Key, out var parentTypeProperty))
                        {
                            return new TypeProperty(
                                parentTypeProperty.Name,
                                UnionType.Create(group.Select(p => typeManager.GetTypeInfo(p))),
                                parentTypeProperty.Flags,
                                parentTypeProperty.Description);
                        }

                        return new TypeProperty(group.Key, UnionType.Create(group.Select(p => typeManager.GetTypeInfo(p))));
                    });
